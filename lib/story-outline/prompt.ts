...
2. 输出严格的 JSON 数组，不要输出 markdown，不要输出解释文字，不要输出代码块。
3. 每个数组元素都必须包含以下字段：
   - "title": 场景标题，简洁明确。
...
4. 所有场景必须按照时间升序排列，且必须以 SRT 的第一句开始时间作为第一个场景开始时间，以 SRT 的最后一句结束时间作为最后一个场景结束时间。
5. 相邻场景之间的时间必须连续衔接：前一个场景的 endSeconds 必须等于后一个场景的 startSeconds，中间不能有空白时间，也不能重叠。
6. 切分场景时优先依据 SRT 的时间线和内容转折，不要跳过任何一段字幕覆盖的时间。
7. 不要编造字幕中完全不存在的核心情节；可以结合剧情简介进行归纳，但不能脱离素材内容。
8. 如果素材信息不足，也必须返回 JSON 数组；至少输出 1 个场景，但不要伪造太多细节。
`;
